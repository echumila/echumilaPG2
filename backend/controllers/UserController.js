...
function editar(req, res){
    ...
    if(data.password){
        bcrypt.hash(data.password, null, null, function(err, hash){
            if(hash){
                user.findByIdAndUpdate(id,{nombres: data.nombres, password: hash, email: 
                    data.email, role: data.role},(err,user_edit)=>{
                        if(user_edit){
                            res.status(200).send({user: user_edit});
                        }else{
                            res.status(500).send({message: 'No se edito el usuario'});
                        }
                    })
            }
        });
        ...
        bcrypt.hash(data.password, null, null, function(err, hash){
            if(hash){
                user.findByIdAndUpdate(id,{nombres: data.nombres, email: 
                    data.email, role: data.role},(err,user_edit)=>{
                        if(user_edit){
                            res.status(200).send({user: user_edit});
                        }else{
...
function get_user(req, res){    
    ...
    User.findById(id,(err,user_data)=>{
        if(user_data){
            ...
            res.status(403).send({message: 'No se encontro el usuario'});
        }
    })
            ...
            if(user_data){
                bcrypt.compare(data.password,user_data.password, function(err, check){
                    if(check){
                        ...
                        res.status(403).send({message: 'El correo o contraseña con coinciden'});
                    }
                });
            }else{
